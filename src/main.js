...
const AI_HISTORY_PATH = window.DATA_STORAGE + "gemini";
...
class Gemini {
  ...
  async init($page) {
    ...
    editor.commands.addCommand({
      name: "gemini_update_token",
      description: "Update Gemini Token",
      exec: this.updateApiToken.bind(this),
    });
    
    $page.id = "acode-plugin-gemini";
    $page.settitle("Gemini");
    this.$page = $page;
    ...
    acode.addIcon('gemini_ai_img', this.baseUrl + 'assets/gemini_avatar.svg');
    sidebarApps.add('gemini_ai_img', 'dall-e-ai', 'Image Generator AI', (app) => {
  ...
  async run() {
    ...
    */
    try {
      let token;
      const myOpenAiToken = window.localStorage.getItem("gemini-api-key");
      if (myOpenAiToken) {
        token = myOpenAiToken;
      } else {
  ...
  async updateApiToken() {
    ...
    */
    ...
    window.localStorage.setItem("gemini-api-key", newApiToken["token"]);
      ...
      if (CURRENT_SESSION_FILEPATH == null) {
        try {
          ...
          if (!await fs(AI_HISTORY_PATH).exists()) {
            await fs(window.DATA_STORAGE).createDirectory("gemini");
          }
  ...
  async sendQuery() {
    ...
    */
    ...
    if (chatText.value != "") {
      ...
      this.loader();
      this.getGeminiResponse(chatText.value);
      chatText.value = "";
    }
  }
  ...
  async appendGptResponse(message) {
    ...
    */
    const gemini_avatar = this.baseUrl + "assets/gemini_avatar.svg";
  ...
  responseMimeType: "text/plain",
       ...
       const gem = await this.$gemini.startChat({
    generationConfig,
    history: arrMessage,
  });
      ...
      const res = await gem.sendMessage(question)
      // remove dot loader 
      clearInterval(this.$loadInterval);
      const targetElem = responseBox[responseBox.length - 1];
      let result = res.response.text();
  ...
  async destroy() {
    editorManager.editor.commands.removeCommand("gemini");
    editorManager.editor.commands.removeCommand("gemini_update_token");
    window.localStorage.removeItem('gemini-api-key');
